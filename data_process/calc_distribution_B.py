...
def process_file(nt, file, nvar, nx, ny, nz, xgrid, ygrid, zgrid):
    ...
    if os.path.exists('./output/Babs_ds_{:03d}.npy'.format(nt)):
        ...
        tmp, rho = read_output_onevariable(file, 0, nvar, nx, ny, nz)
        tmp, vx = read_output_onevariable(file, 1, nvar, nx, ny, nz)
        tmp, vy = read_output_onevariable(file, 2, nvar, nx, ny, nz)
        tmp, vz = read_output_onevariable(file, 3, nvar, nx, ny, nz)
        tmp, Bx = read_output_onevariable(file, 4, nvar, nx, ny, nz)
        tmp, By = read_output_onevariable(file, 5, nvar, nx, ny, nz)
        tmp, Bz = read_output_onevariable(file, 6, nvar, nx, ny, nz)
        tmp, p = read_output_onevariable(file, 7, nvar, nx, ny, nz)
